...
export function VerticalScoreTable({
  ...
  const calculateTotalScore = (player: PlayerScore) => {
...
// Combined table showing all 18 holes with OUT, IN, and TOTAL
interface FullScoreTableProps {
  holes: HoleInfo[];
  players: PlayerScore[];
  onCellClick: (playerId: string, holeNumber: number) => void;
  scoreDisplayMode: ScoreDisplayMode;
...
export function FullVerticalScoreTable({
  holes,
  players,
  onCellClick,
  scoreDisplayMode,
  ...
  const getScoreStyle = (score: number | null, par: number) => {
    ...
    if (diff === -1) return 'bg-red-100 text-red-700 font-bold';
    if (diff === 0) return 'bg-green-100 text-green-700';
    if (diff === 1) return 'bg-blue-100 text-blue-700';
    if (diff === 2) return 'bg-blue-200 text-blue-800';
  ...
  const calculateTotal = (player: PlayerScore, holeSet: HoleInfo[]) => {
  ...
  const calculatePar = (holeSet: HoleInfo[]) => {
    return holeSet.reduce((sum, hole) => sum + hole.par, 0);
  };
  ...
  const hasScoresInSet = (player: PlayerScore, holeSet: HoleInfo[]) => {
    return holeSet.some((h) => player.scores[h.hole] !== undefined);
  };
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b bg-muted/30">
            <th className="py-2 px-2 text-center font-medium text-muted-foreground w-12 sticky left-0 bg-muted/30">
              홀
            </th>
            <th className="py-2 px-2 text-center font-medium text-muted-foreground w-10">
              파
        ...
        <tbody>
          ...
          {frontNine.map((hole) => (
            <tr key={hole.hole} className="border-b">
              <td className="py-2.5 px-2 text-center font-medium sticky left-0 bg-background">
                {hole.hole}
              </td>
              <td className="py-2.5 px-2 text-center text-muted-foreground">
                {hole.par}
          ...
          {/* OUT Row */}
          <tr className="border-b-2 bg-muted/50 font-medium">
            <td className="py-2.5 px-2 text-center sticky left-0 bg-muted/50">OUT</td>
            <td className="py-2.5 px-2 text-center text-muted-foreground">{frontPar}</td>
            {players.map((player) => {
              const frontTotal = calculateTotal(player, frontNine);
              const hasScores = hasScoresInSet(player, frontNine);
              ...
              return (
                ...
                >
                  {formatTotalScore(frontTotal, frontPar, hasScores)}
                </td>
              );
            })}
          </tr>
          ...
          {/* IN Row */}
          <tr className="border-b bg-muted/50 font-medium">
            <td className="py-2.5 px-2 text-center sticky left-0 bg-muted/50">IN</td>
            <td className="py-2.5 px-2 text-center text-muted-foreground">{backPar}</td>
            {players.map((player) => {
              const backTotal = calculateTotal(player, backNine);
              const hasScores = hasScoresInSet(player, backNine);
              const diff = backTotal - backPar;
              return (
                <td
                  key={`${player.id}-in`}
                  className={cn(
                ...
                >
                  {formatTotalScore(backTotal, backPar, hasScores)}
                </td>
              );
            })}
          </tr>
          ...
          <tr className="bg-primary/10 font-bold">
            <td className="py-3 px-2 text-center sticky left-0 bg-primary/10">TOT</td>
            <td className="py-3 px-2 text-center text-muted-foreground">{totalPar}</td>
            {players.map((player) => {
              const total = calculateTotal(player, holes);
              const hasScores = hasScoresInSet(player, frontNine) || hasScoresInSet(player, backNine);
              const diff = total - totalPar;
              return (
                <td
                  key={`${player.id}-total`}
                  className={cn(
                    'py-3 px-2 text-center text-lg',
                    hasScores && diff > 0 && 'text-blue-600',
                    hasScores && diff < 0 && 'text-red-600',
                    hasScores && diff === 0 && 'text-green-600'
                  )}
                >
                  {formatTotalScore(total, totalPar, hasScores)}
                </td>
              );
            })}
          </tr>
